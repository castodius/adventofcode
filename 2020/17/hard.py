        ...
        for zd in deltas:
          for wd in deltas:
            if xd == 0 and yd == 0 and zd == 0 and wd == 0:
              continue
            active += 1 if self.matrix[x+xd][y+yd][z+zd][w+wd] == ACTIVE else 0
    return active
...
def init():
  ...
  for index, row in enumerate(rows):
    matrix.matrix[SHIFT][SHIFT][SHIFT+index][SHIFT:SHIFT+n] = [char for char in row]
    ...
    for y in range(1, side-1):
      for z in range(1, side-1):
        for w in range(1, side-1):
          count = matrix.get_active_neighbours(x, y, z, w)
          if matrix.matrix[x][y][z][w] == ACTIVE:
            if 2 <= count <= 3:
              new_matrix.matrix[x][y][z][w] = ACTIVE
          else:
            if count == 3:
              new_matrix.matrix[x][y][z][w] = ACTIVE
